...
\begin{theorem}[One-Sided Error Reduction with One Random Seed]
	...
	Let \(A(x,r)\) be a one-sided randomized algorithm using \(m\) random bits, and suppose that for every \(x\notin L\),
	\begin{equation}
		\label{eq:magical-error-original}
		\Pr_{r\sample\bits^m}[A(x,r)=1]\leq \frac{1}{17}.
	\end{equation}
	Let \(\Graph=(\LeftSet,\RightSet,\Edges)\) be an \((n,n;d)\)-magical graph with \(n=2^m\), and identify both \(\LeftSet\) and \(\RightSet\) with \(\bits^m\). The algorithm that chooses one uniform \(r\in\LeftSet\), evaluates \(A(x,r')\) on all \(d\) neighbors \(r'\) of \(r\), and accepts if and only if all \(d\) evaluations accept has one-sided error at most \(1/(10d)+1/n\).
\end{theorem}
